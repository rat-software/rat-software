...
def get_keywords_bg():
    conn = psycopg2.connect(database=database, user=user, password=password, host=host, port=port)  # Connection to the database
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute("SELECT keyword, qs_study_id as study_id, qs_language_code.name AS language, qs_geotarget.name AS region, qs_language_code_criterion_id AS language_criterion_id, qs_geotarget_criterion_id AS region_criterion_id, qs_keyword.id AS keyword_id FROM qs_keyword, qs_language_code, qs_geotarget WHERE qs_language_code_criterion_id = qs_language_code.criterion_id AND qs_geotarget_criterion_id = qs_geotarget.criterion_id AND qs_keyword.status = 0 AND qs_keyword.keyword !='' ORDER BY qs_keyword.id;")
    conn.commit()
    keywords = cur.fetchall()  # Fetch all keywords
    conn.close()
    return keywords    
