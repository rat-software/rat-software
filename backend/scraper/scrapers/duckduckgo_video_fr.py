...
def run(query, limit, scraping, headless):
    ...
    try:
        ...
        def get_search_results(driver, page):
            ...
            for result in soup.find_all("div", class_=lambda x: x and "opt--t-xxs" in x):
                try:
                    result_title = result.find('h6', class_='tile__title').find('a').text
                except:
                    result_title = "N/A"

                try:

                    footer_div = result.find('div', class_='tile__body__footer')
                ...
                try:
                    result_url = result.find('h6', class_='tile__title').find('a')['href']
                except:
                    result_url = "N/A"
        ...
        try:
            ...
            dropdown.select_by_value('fr-fr')
        ...
        if not check_captcha(driver):
            ...
            video_tab = driver.find_element(By.XPATH, "//a[contains(@href, 'ia=videos&iax=videos')]")
