...
def run(query, limit, scraping, headless):
    try:
        ...
        search_url = "https://www.mojeek.de/" #URL of search engine, e. g. www.google.de
        ...
        next_page = "Next" #CSS to find click on next SERP; for search engines that use a navigation on SERPS to continue browsing more search results
        ...
        def get_search_results(driver, page):
            ...
            for outer in soup.find_all("ul", class_=["results-standard"]):
                ...
                #try to extract the title based on a css class for the link title
                    try:
                        for title in result.find("a", class_=["title"]):
                    ...
                    #try to extract the result description by css class and change the content of the description, if necessary:
                        ...
                        for description in result.find_all("p", class_=["s"]):
                ...
                #try to extract the urls of the seaech results and change the url if the search engines works with relative hyperlinks
                    try:
                        for url in result.find_all("a", class_=["title"]):
                            url = url.attrs['href']
                            url_list.append(url)
