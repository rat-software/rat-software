...
def format_answer(ai_container):
    ...
    filter_text = ["Opens in new tab", "Alle anzeigen"]
...
def run(query, limit, scraping, headless):
    ...
    try:
        ...
        # Versuch, den AI-Container zu finden (primäre Klasse)
        ai_container = None
        try:
            ai_container = driver.find_element(By.CSS_SELECTOR, "div.LT6XE")
            print("Primary AI Overview element found.")
        except NoSuchElementException:
            print("Primary AI Overview element not found. Trying fallback element.")
            
        # Versuch, den Fallback-AI-Container zu finden
        if not ai_container:
            try:
                ai_container = driver.find_element(By.CSS_SELECTOR, "div.LT6XE")
                print("Fallback AI Overview element found.")
            except NoSuchElementException:
                print("No AI Overview element or fallback element found.")
                return -1

        # Wenn ein Container gefunden wurde, verarbeiten wir ihn
        if ai_container:
...
if __name__ == "__main__":
    query = "wie behandelt man migräne?"
    headless = False
    limit = 1
    scraping = False
