...
def run(query, limit, scraping, headless):
    try:
        ...
        search_url = "https://duckduckgo.com/" #URL of search engine, e. g. www.google.de
        search_box = "q" #Class name of search box; input field for searches
        captcha = "g-reaptcha" #Source code hint for CAPTCHA; some search engines use CAPTCHAS to block too many automatic requests
        next_page = "//button[@id='more-results']" #CSS to find click on next SERP; for search engines that use a navigation on SERPS to continue browsing more search results
        ...
        def get_search_results(driver, page):
            ...
            for result in soup.find_all("li", class_=["wLL07_0Xnd1QZpzpfR4W"]):
                ...
                try:
                    for title in result.find("span", class_=["EKtkFWMYpwzMKOYr0GYm LQVY1Jpkk8nyJ6HBWKAk"]):
                ...
                try:
                    ...
                    for description in result.find_all("div", class_=["E2eLOJr8HctVnDOTM8fs"]):
                ...
                try:
                    for url in result.find_all("a", class_=["Rn_JXVtoPVAFyGkcaXyK"]):
