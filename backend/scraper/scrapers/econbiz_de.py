...
def run(query, limit, scraping, headless):
    ...
    try:
        ...
        search_url = "https://www.econbiz.de/" #URL of search engine, e. g. www.google.de
        ...
        next_page = "//span[@class='icon-inline econ-right']" #CSS to find click on next SERP; for search engines that use a navigation on SERPS to continue browsing more search results
        ...
        def get_search_results(driver, page):
            ...
            counter = 0  #adjusts the number of results in list
            ...
            for result in soup.find_all("div", class_=["col-sm-10 col-xs-9 result-content"]):  
                ...
                try:
                    for title in result.find_all("span", property= ["schema:headline"]):
                ...
                try:
                    ...
                    for description in result.find_all("div", class_=["list-metadata", "snippets"]):
                ...
                try:
                    for url in result.find_all("a", class_=["title print-no-url"]):
                        url = url.attrs['href']
                        #url.replace("Persistenter Link:", " ")
                        url_list.append(f"https://www.econbiz.de{url}")
