...
def run(query, limit, scraping, headless):
    try:
        ...
        next_page = "Nächste" #CSS to find click on next SERP; for search engines that use a navigation on SERPS to continue browsing more search results
        ...
        def get_search_results(driver, page):
            ...
            serp_code = scraping.encode_code(driver.page_source) #use the function from the lib scraping to encode the source code to base64; preparing it to be stored in database
            ...
            for result in soup.find_all("div", class_= lambda x: x and x.startswith('snippet svelte')):
                ...
                try:
                    for title in result.find("div", class_=re.compile("heading-serpresult", re.I)):  
            ...
            if results_number > 0:
                ...
                while (results_number < limit) and continue_scraping:
                    if not check_captcha(driver):
                        ...
                        try:
                            next = driver.find_element(By.LINK_TEXT, next_page)
                            next.click()
                            search_results+= get_search_results(driver, page)
                            results_number = len(search_results)
