...
class DB:
    ...
    def get_sources_pending(self, job_server):
        ...
        conn = DB.connect_to_db(self)
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cur.execute("SELECT result_source.id, result_source.source, result_source.result  FROM result_source where (result_source.progress = 2 or result_source.progress = -1)  and result_source.counter < 11 and result_source.job_server = %s and result_source.created_at < now() - interval '10 minutes'",(job_server,))
        conn.commit()
    ...
    def update_sources_failed(self, job_server):
        """
        Get all finally failed sources (progress = 2 and counter > 11)
        """
        conn = DB.connect_to_db(self)
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cur.execute("SELECT result_source.source  FROM result_source where (result_source.progress = 2 or result_source.progress = -1 or result_source.progress = 0)  and result_source.counter > 11 and result_source.job_server = %s",(job_server,))
        conn.commit()
    ...
    def get_sources(self, job_server):
        ...
        sql = """
            ...
            WHERE (rs.source IS NULL OR (rs.progress = 0 AND rs.counter < 11)) 
            AND s.studytype != 6 
            ORDER BY r.id ASC 
            LIMIT 5;
        """
