...
def qs_download_template(template_type):
    """
    Generiert und liefert eine CSV-Vorlage für Keywords zum Download.
    """
    si = StringIO()
    cw = csv.writer(si)
...
def preview_qs_query_csv():
    """
    Nimmt eine hochgeladene Query-CSV-Datei entgegen, validiert sie
    und gibt eine Vorschau der ersten 10 Zeilen als JSON zurück.
    """
...
def new_qs_study_wizard():
    """
    Erstellt eine neue Query Sampler Studie auf einer einzigen Seite.
    """
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            db.session.commit() # Commit, um die ID zu generieren

            # 3. Keywords speichern
