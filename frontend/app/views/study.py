...
def upload_to_storage(file_data, filename):
    base_url = app.config.get('STORAGE_BASE_URL', 'https://tool.rat-software.org')
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            if serp_key not in serp_cache:
                # 1. Zuerst prüfen, ob überhaupt Dateien (HTML/JPG) im ZIP vorliegen
                has_content = False
                img_match = None
                html_match = None
                ...
                # 2. NUR wenn Inhalt gefunden wurde, legen wir den Serp-Datensatz an!
                if has_content:
                    ...
                    if not serp:
                        ...
                        db.session.add(serp)
                        db.session.flush() # Wichtig, damit die ID generiert wird
                    ...
                    # Kein Bild/HTML gefunden -> Wir erzeugen KEINEN Serp-Eintrag!
            ...
            if res_type == 'organic':
                ...
                if url:
                    # FIXED: db.session.query() statt Result.query... verwendet
                ...
                if ai_text:
                    # FIXED: db.session.query() statt ResultAi.query... verwendet
                ...
                if url and parent_ai_id:
                    # FIXED: db.session.query() statt ResultAiSource.query... verwendet
...
def study(id):
    study = Study.query.get_or_404(id)
    # HIER NEU: Wir fangen den progress_percent ab, um ihn direkt beim ersten Laden ans Template zu geben!
    status, progress_percent = check_and_update_status(study)
...
def update_study_settings(id):
    ...
    if form.validate_on_submit():
        ...
        # --- 1. LIVE MODE SPEICHERN ---
        study.live_link_mode = form.live_link_mode.data
        
        # --- 2. ZWANGS-LOGIK FÜR SHOW URLs ---
        if study.live_link_mode:
            study.show_urls = True  # Im Live Mode MUSS die URL sichtbar sein!
        else:
            study.show_urls = form.show_urls.data # Sonst greift die normale Auswahl
...
def delete_study(id):
    ...
    if form.validate_on_submit():
        try:
            ...
            storage_dir = '/var/www/rat/storage/sources/'
            deleted_count = 0
